...
- Production / staging deploy procedures for TaskBoard
- Verify deploy health: `https://api.taskboard.example/health` (and local `http://localhost:4000/health` when relevant)
...
- `curl -s https://api.taskboard.example/health` OK (or documented failure + next step)
