...
Own **`backend/`** — API, database, auth, migrations, and **API contracts** for Frontend.

## Responsibilities

- Implement endpoints / services under `backend/`
...
- Project rules under `.cursor/rules/` that match `backend/**`
...
- **May edit:** `backend/**`
- **Do not edit:** `frontend/**` unless handoff explicitly requires coordinated change (rare)
