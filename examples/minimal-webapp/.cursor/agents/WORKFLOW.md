# Virtual team workflow — TaskBoard
...
2. **Backend** (if API/DB/auth) — Implement in `backend/`. Update `backend/STATE.md`. Add API contract notes to handoff.
3. **Frontend** (if UI/client) — Implement in `frontend/`. Consume Backend handoff. Update `frontend/STATE.md`.
...
| Owns | Path / artifact |
|------|-----------------|
| Backend | `backend/**`, API contracts in handoffs |
| Frontend | `frontend/**` |
...
curl -s https://api.taskboard.example/health
